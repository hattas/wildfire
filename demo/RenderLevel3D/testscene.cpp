...
void myKeyboard(unsigned char theKey, int mouseX, int mouseY) {
	...
	case 'f':
		...
		break;
	/*
	//unit selection to add new piece to the board
	case '1': //generate truck and flag that new truck is selected
		TruckPieces[truckCount].x = globalMouseX;
		TruckPieces[truckCount].y = globalMouseY;
		TruckPieces[truckCount].z = globalMouseZ;
		selectedPiece = truck;
		break;
	case '2': //generate helicopter and flag that helicopter is selected
		ChopperPieces[chopperCount].x = globalMouseX;
		ChopperPieces[chopperCount].y = globalMouseY;
		ChopperPieces[chopperCount].z = globalMouseZ;
		selectedPiece = chopper;
		break;
	case '3': //generate helicopter and flag that helicopter is selected
		BoatPieces[boatCount].x = globalMouseX;
		BoatPieces[boatCount].y = globalMouseY;
		BoatPieces[boatCount].z = globalMouseZ;
		selectedPiece = chopper;
		break;
	*/
	default: 
...
void initGame() {
	...
	glutCreateWindow("Wild and Smokey: Game");
...
void homeMouse(int button, int state, int x, int y) {
	if (state == GLUT_DOWN) {
		if (button == GLUT_LEFT_BUTTON)
			...
			case(HOW_TO_PLAY): gameState = startPageClick(button, x, y); cout << gameState << endl; break;
			case(ABOUT): gameState = startPageClick(button, x, y); cout << gameState << endl; break;
			case(PLAY_GAME): break;
			}
		homeDisplay();
	}
